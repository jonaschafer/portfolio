...
function AudioPlayer({ youtubeId, onPlay, isActive }) {
  ...
  useEffect(() => {
    // Stop playing if another track becomes active
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div id={`player-${youtubeId}`} className="hidden" />
    </div>
  )
}
...
function SongCard({ song, isActive, onPlay }) {
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}/play/music-blog#${song.id}`
    ...
    <div className="font-mono bg-white text-black min-h-screen">
      ...
      <header className="border-b-2 border-black">
        <div className="min-w-[375px] max-w-[1440px] mx-auto px-5 md:px-[60px] py-8 md:py-12">
          ...
          <p className="font-['Haas_Grot_Disp',_sans-serif] text-[14px] md:text-[16px] text-black/70 mt-3 max-w-[500px]">
            Music worth sharing. Click play for audio, or find it on your preferred service.
          </p>
        </div>
      </header>
